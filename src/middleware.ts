...
export async function middleware(request: NextRequest) {
    ...
    const cookieDomain = env === "production" ? ".speck.ing" : ".staging.speck.ing";

    if (isProdLike) {
        // Skip static and API files
        if (
            pathname.startsWith('/_next') ||
            pathname.startsWith('/api') ||
            pathname.match(/\.(png|jpg|jpeg|svg|ico|webp|js|css|woff|ttf|map)$/)
        ) {
            return NextResponse.next();
        }
        ...
        const publicRoutes = ['/auth', '/'];
        const protectedRoutes = ['/home', '/profile', '/library', '/create', '/templates, /allmaps'];
...
export const config = {
    matcher: ["production", "staging"].includes(process.env.NEXT_ENV ?? "")
        ? matcherProd
        : matcherDev,
};
